...
export class PaymentsController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Payment successful. Access granted to the blog.',
    type: PaymentResponseDto,
  })
  @ApiResponse({
    status: 400,
    description: 'Bad request, e.g., insufficient balance or already paid',
  })
  @ApiResponse({
    status: 404,
    description: 'Not found, e.g., user or blog not found',
  })
  @UseGuards(JwtAuthGuard) // Apply JWT authentication guard
  async payToView(
    @Body() paymentDto: PayToViewDto,
